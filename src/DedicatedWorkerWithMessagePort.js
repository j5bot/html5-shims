...
if (typeof Worker === "undefined" || Worker.prototype.constructor === Worker) {
	Worker = (function InitWorker(window,navigator,wgsSource){
		...
		sender -- messages are sent by this port, it's the one where postMessage is implemented
		receiver -- messages are received by this port, it's the one where onmessage is implemented
		
		*/
		function MessagePort(sender,receiver) {
			var ready = false,
				receiver = receiver,
				sender = sender,
				inboundMessages = [],
				outboundMessages = [],
				interval = setInterval(function () { this._sendMessages(); },10);
		}
				...
				if (ready) {
					...
					if (sender.postMessage && outboundMessages.length > 0) {
						while (outboundMessages.length > 0) {
							sender.postMessage(shift(outboundMessages));
						}
					}
				}
			}
